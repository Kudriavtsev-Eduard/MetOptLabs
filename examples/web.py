...
from src.scheduler import Scheduler, DichotomyScheduler


def get_default_optimizer():
    scheduler: Scheduler = DichotomyScheduler(1, 3)  # Set learning rate strategy
    break_checker: BreakChecker = ArgumentAbsoluteBreakChecker(10 ** -5)  # Set breakpoint condition checking
    optimizer: GradientOptimizer = GradientOptimizer(scheduler, break_checker,
                                                     limit=10)  # Combine all above into gradient optimizer
    return optimizer
    ...
    func: DerivableFunction = AutomatedDerivableFunction(
        CachedFunction(
            lambda x, y: client.get_elevation(min_max(x, LOWER_BOUND, UPPER_BOUND),
                                              min_max(y, LOWER_BOUND, UPPER_BOUND))))
    start_point: tuple[float, float] = (42, 42)
